...
public class GUI_Button extends GUIElement
{
    ...
    {
        this.hb = hitbox;
        this.background = pinacolada.utilities.PCLRenderHelpers.ForTexture(buttonTexture);
    ...
    {
        this.background = PCLRenderHelpers.ForTexture(borderTexture);
    ...
    public GUI_Button SetBorder(Texture borderTexture, Color color)
    {
        ...
        {
            this.border = pinacolada.utilities.PCLRenderHelpers.ForTexture(borderTexture, color).SetHitbox(hb);
        }
    ...
    {
        return SetTooltip(new PCLCardTooltip(title, description));
    }

    public GUI_Button SetTooltip(PCLCardTooltip tooltip)
    {
    ...
    public void Update()
    {
        ...
        if (IsInteractable() && PGR.UI.TryHover(hb))
        {
            ...
            if (this.hb.hovered)
            {
                if (currentClickDelay <= 0)
                {
                    if (PCLInputManager.RightClick.IsJustPressed())
                    {
                        OnRightClick();
                    }
    ...
    public void Render(SpriteBatch sb)
    {
        ...
        if (StringUtils.isNotEmpty(text))
        {
            ...
            if (isSmartText) {
                PCLRenderHelpers.WriteSmartText(sb, font, text, hb.x + (hb.width * 0.5f), hb.y + (hb.height * 0.5f), hb.width, font.getLineHeight(), color);
            }
            else if (FontHelper.getSmartWidth(font, text, 9999f, 0f) > (hb.width * 0.7))
            {
                pinacolada.utilities.PCLRenderHelpers.WriteCentered(sb, font, text, hb, color, 0.8f);
            }
            else
            {
                pinacolada.utilities.PCLRenderHelpers.WriteCentered(sb, font, text, hb, color);
            }
