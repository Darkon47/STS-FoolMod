package pinacolada.ui.hitboxes;
    ...
    public void update()
    {
        ...
        if (dragStart != null)
        {
            if (Settings.isDebug)
            {
                ...
                PCLJUtils.LogInfo(this, "x  = {0}({1}%) , y  = {2}({3}%)", x, xPercentage, y, yPercentage);
                PCLJUtils.LogInfo(this, "cX = {0}({1}%) , cY = {2}({3}%)", cX, cxPercentage, cY, cyPercentage);
            }
