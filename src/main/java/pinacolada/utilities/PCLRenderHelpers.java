...
public class PCLRenderHelpers extends STSRenderUtils
{
    ...
    public static BitmapFont GetLargeAttributeFont(PCLCard card)
    {
        BitmapFont result;
        if (card.isPopup)
        {
    ...
    public static BitmapFont GetSmallAttributeFont(PCLCardBase card)
    {
        BitmapFont result;
        if (card.isPopup)
        {
    ...
    public static BitmapFont GetSmallTextFont(PCLCardBase card, String text)
    {
    ...
    public static BitmapFont GetSmallTextFont_Legacy(PCLCardBase card, String text)
    {
        float scaleModifier = 0.8f;
        int length = text.length();
        if (length > 20)
    ...
    public static BitmapFont GetDescriptionFont(PCLCardBase card, float scaleModifier)
    {
        BitmapFont result;
        if (card.isPopup)
        {
    ...
    public static BitmapFont GetTitleFont(PCLCardBase card)
    {
        BitmapFont result;
    ...
    public static BitmapFont GetEnergyFont(PCLCardBase card)
    {
        BitmapFont result;
        if (card.isPopup)
        {
    ...
    {
        if (img == null) {
            PCLJUtils.LogWarning(card, "Image was null:");
            return;
        }
        final float scale = card.drawScale * Settings.scale * imgScale;
    ...
    {
        DrawOnCard(sb, card, PCLColors.White(card.transparency), img, drawX, drawY, size, size);
    }
    ...
    public static void WriteOnCard(SpriteBatch sb, AbstractCard card, BitmapFont font, String text, float x, float y, Color color, boolean roundY)
    {
        ...
        color = PCLColors.Copy(color, color.a * card.transparency);
    ...
    public static ColoredString GetCardAttributeString(PCLCard card, char attributeID)
    {
        switch (attributeID)
        {
            case 'D':
    ...
    private static Color GetColor(Character c, Color baseColor)
    {
        switch (c)
        {
            ...
            default:
                PCLJUtils.LogWarning(pinacolada.utilities.PCLRenderHelpers.class, "Unknown color: #" + c);
                return baseColor.cpy();
        }
    }

    public static float GetTooltipHeight(PCLCardTooltip tip)
    {
        return -GetSmartHeight(FontHelper.tipBodyFont, tip.description, BODY_TEXT_WIDTH, TIP_DESC_LINE_SPACING) - 7.0F * Settings.scale;
    }

    public static float CalculateAdditionalOffset(ArrayList<PCLCardTooltip> tips, float hb_cY)
    {
    ...
    public static float CalculateToAvoidOffscreen(ArrayList<PCLCardTooltip> tips, float hb_cY)
    {
        return tips.isEmpty() ? 0f : Math.max(0.0F, GetTallestOffset(tips) - hb_cY);
    }

    private static float GetTallestOffset(ArrayList<PCLCardTooltip> tips)
    {
        float currentOffset = 0f;
        float maxOffset = 0f;

        for (PCLCardTooltip p : tips)
        {
