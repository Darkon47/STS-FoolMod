...
{
    public static final PCLCardData DATA = Register(ShuichiSaihara.class).SetSkill(0, CardRarity.UNCOMMON, PCLCardTarget.None)
            .SetColorless()
            .SetSeries(CardSeries.Danganronpa)
        ...
        public void onExhaust(AbstractCard card)
        {
            ...
            if (card.uuid.equals(this.card.uuid) && !AbstractDungeon.getMonsters().areMonstersBasicallyDead()) {
                ...
                while (choice.size() < 3 && pool.Size() > 0)
                {
                    ...
                    if (!(temp.cardID.equals(card.cardID) || temp.tags.contains(AbstractCard.CardTags.HEALING) || temp.tags.contains(PGR.Enums.CardTags.VOLATILE))) {
