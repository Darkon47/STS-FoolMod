...
public class PCLDungeonData implements CustomSavable<PCLDungeonData>, StartGameSubscriber, StartActSubscriber
{
    ...
    {
        return eatyourbeets.resources.GR.IsLoaded() ? PGR.Common.Dungeon.IsUnnamedReign() : EnteredUnnamedReign;
    }
    ...
    public void OnCardObtained(AbstractCard card)
    {
        ...
        if (card.tags.contains(PGR.Enums.CardTags.UNIQUE))
        {
    ...
    public boolean CanObtainCopy(AbstractCard card) {
        final PCLCard pclCard = JUtils.SafeCast(card, PCLCard.class);
